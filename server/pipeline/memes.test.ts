  ...
  const fakeFetch = (async (url: string) => {
    if (url.includes("api.giphy.com")) {
      return new Response(JSON.stringify({
        data: [{ images: { original: { mp4: "https://giphy.example/clip.mp4" } } }],
      }));
    }
    if (url === "https://giphy.example/clip.mp4") {
...
test("fetchMemeAsset returns null when Giphy returns no results", async () => {
  const dir = mkdtempSync(path.join(tmpdir(), "memes-test-"));
  const fakeFetch = (async () => new Response(JSON.stringify({ data: [] }))) as unknown as typeof fetch;
