...
## Temporary Feature: Direct YouTube Upload

> [!IMPORTANT]
> **Temporary Solution Notice**: A direct YouTube upload feature has been added to the Job details page as a temporary solution. It uses single-user `.env` credentials (`YOUTUBE_CLIENT_ID`, `YOUTUBE_CLIENT_SECRET`, `YOUTUBE_REFRESH_TOKEN`).
>
> **Future Phase Migration**: In Phase 15 (YouTube Upload & Channel Linking), this temporary single-channel implementation will be completely replaced with full multi-tenant YouTube OAuth channel management and quota ledger tracking.

### Configuring Direct YouTube Upload (.env)

To enable the "Upload to YouTube" button on rendered clips, add the following to your `.env` file:

```env
YOUTUBE_CLIENT_ID="your-google-oauth-client-id"
YOUTUBE_CLIENT_SECRET="your-google-oauth-client-secret"
YOUTUBE_REFRESH_TOKEN="your-google-oauth-refresh-token"
```

When clicked, the button uploads the clip directly to your YouTube channel as an `unlisted` video, automatically populating the title, description, and hashtags generated by the AI pipeline.
